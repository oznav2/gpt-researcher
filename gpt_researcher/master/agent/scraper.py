    ...
    async def scrape_urls(self, urls: List[str]) -> List[Dict]:
        ...
        if self.researcher.verbose:
            await self.researcher.stream_output(
                "logs",
                "scraping_urls",
                f"🌐 מחלץ טקסט רלוונטי {len(urls)} מהלי...",
        ...
        if self.researcher.verbose:
            await self.researcher.stream_output(
                "logs",
                "scraping_complete",
                f"✅  חילוץ הטקסט הסתיים בהצלחה מ {len(scraped_content)} מקורות",
                self.researcher.websocket,
            )
    ...
    async def scrape_data_by_query(self, query: str) -> List[Dict]:
        ...
        if self.researcher.verbose:
            await self.researcher.stream_output(
                "logs",
                "searching_query",
                f"🔍 מאתר מקורות רלוונטים לנושא המחקר: '{query}'...",
                self.researcher.websocket,
            )
        ...
        if self.researcher.verbose:
            await self.researcher.stream_output(
                "logs",
                "scraping_query_urls",
                f"🌐 מחלץ תוכן מ {len(new_search_urls)} מקורות שנמצאו עבור הנושא: '{query}'...",
                self.researcher.websocket,
            )
