    ...
    async def conduct_research(self):
        ...
        if self.researcher.verbose:
            await stream_output(
                "logs",
                "starting_research",
                f"🔎 מבצע מחקר בנושא '{self.researcher.query}'...",
        ...
        if self.researcher.verbose:
            await stream_output(
                ...
                f"Finalized research step.\n💸 עלות משוערת למחקר: ${self.researcher.get_costs()}",
                self.researcher.websocket,
            )
    ...
    async def __get_context_by_urls(self, urls):
        ...
        if self.researcher.verbose:
            await stream_output(
                "logs",
                "source_urls",
                f"🗂️ המחקר שלי בין היתר יתבסס על המקורות הבאים: {new_search_urls}...",
                self.researcher.websocket,
            )
    ...
    async def __get_context_by_vectorstore(self, query, filter: Optional[dict] = None):
        ...
        if self.researcher.verbose:
            await stream_output(
                "logs",
                "subqueries",
                f"🗂️  המחקר שלי ינתח בין היתר את השאלות הבאות: {sub_queries}...",
                self.researcher.websocket,
                True,
                sub_queries,
            )
    ...
    async def __get_context_by_search(self, query, scraped_data: list = []):
        ...
        if self.researcher.verbose:
            await stream_output(
                "logs",
                "subqueries",
                f"🗂️ אבצע מחקר בנושאים של השאלות הבאות: {sub_queries}...",
                self.researcher.websocket,
                True,
                sub_queries,
            )
    ...
    async def __process_sub_query_with_vectorstore(self, sub_query: str, filter: Optional[dict] = None):
        ...
        if content and self.researcher.verbose:
            ...
            await stream_output(
                ...
                f"🤷 לא מצאתי תוכן בנושא '{sub_query}'...",
                self.researcher.websocket,
            )
        return content
    ...
    async def __scrape_data_by_query(self, sub_query):
        ...
        if self.researcher.verbose:
            await stream_output(
                "logs",
                "researching",
                f"🤔 חוקר את הנושא מהמקורות הרלוונטיים הבאים...\n",
                self.researcher.websocket,
            )
