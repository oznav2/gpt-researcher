    ...
    async def write_report(self, existing_headers: list = [], relevant_written_contents: list = [], ext_context=None) -> str:
        ...
        if self.researcher.verbose:
            await stream_output(
                "logs",
                "writing_report",
                f"✍️ כותב דוח מסכם בנושא '{self.researcher.query}'...",
        ...
        if self.researcher.verbose:
            await stream_output(
                "logs",
                "report_written",
                f"📝 נכתב דוח מסכם בנושא '{self.researcher.query}'",
                self.researcher.websocket,
            )
    ...
    async def write_report_conclusion(self, report_content: str) -> str:
        ...
        if self.researcher.verbose:
            await stream_output(
                "logs",
                "writing_conclusion",
                f"✍️ כותב מסקנות המחקר בנושא '{self.researcher.query}'...",
                self.researcher.websocket,
            )
        ...
        if self.researcher.verbose:
            await stream_output(
                "logs",
                "conclusion_written",
                f"📝 מסקנת המחקר בנושא '{self.researcher.query}' נכתבה",
                self.researcher.websocket,
            )
    ...
    async def write_introduction(self):
        ...
        if self.researcher.verbose:
            await stream_output(
                ...
                f"✍️ כותב מבוא לדוח מסכם בנושא '{self.researcher.query}'...",
                self.researcher.websocket,
            )
        ...
        if self.researcher.verbose:
            await stream_output(
                ...
                f"📝 מסיים כתיבת המבוא לדוח מסכם בנושא '{self.researcher.query}'",
                self.researcher.websocket,
            )
    ...
    async def get_subtopics(self):
        ...
        if self.researcher.verbose:
            await stream_output(
                ...
                f"🌳 מייצר תתי נושאים לשאלת המחקר '{self.researcher.query}'...",
                self.researcher.websocket,
            )
        ...
        if self.researcher.verbose:
            await stream_output(
                "logs",
                "subtopics_generated",
                f"📊 תתי נושאים שנוצרו בשאלת המחקר '{self.researcher.query}'",
                self.researcher.websocket,
            )
    ...
    async def get_draft_section_titles(self, current_subtopic: str):
        ...
        if self.researcher.verbose:
            await stream_output(
                ...
                f"📑 מייצר טיוטת כותרות עבור הנושא '{self.researcher.query}'...",
                self.researcher.websocket,
            )
        ...
        if self.researcher.verbose:
            await stream_output(
                ...
                f"🗂️ טיוטת כותרות בנושא הושלמה בנושא '{self.researcher.query}'",
                self.researcher.websocket,
            )
