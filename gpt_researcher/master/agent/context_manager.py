...
class ContextManager:
    ...
    async def __get_context_by_urls(self, urls):
        ...
        if self.researcher.verbose:
            await stream_output(
                "logs",
                "source_urls",
                f"🗂️ את המחקר בנושא המבוקש אבצע בהסתמך על הלינקים הבאים: {new_search_urls}...",
    ...
    async def __get_hybrid_context(self):
        ...
        return f"קונטקסט רלוונטי ממסמכים שהועלו: {docs_context}\n\nקונטקסט רלוונטי מאתרים באינטרנט שנמצאו: {web_context}"
    ...
    async def __get_context_by_vectorstore(self, query, filter: Optional[dict] = None):
        ...
        if self.researcher.verbose:
            await stream_output(
                "logs",
                "subqueries",
                f"🗂️ אבצע מחקר בנושא המבוקש בהסתמך על השאלות הבאות: {sub_queries}...",
                self.researcher.websocket,
                True,
                sub_queries,
            )
    ...
    async def __process_sub_query_with_vectorstore(self, sub_query: str, filter: Optional[dict] = None):
        ...
        if content and self.researcher.verbose:
            ...
            await stream_output(
                ...
                f"🤷 לא נמצא קונטקסט בנושא '{sub_query}'...",
                self.researcher.websocket,
            )
        return content
    ...
    async def __process_sub_query(self, sub_query: str, scraped_data: list = []):
        ...
        if content and self.researcher.verbose:
            ...
            await stream_output(
                "logs",
                "subquery_context_not_found",
                f"🤷 לא נמצא תוכן רלוונטי בנושא '{sub_query}'...",
                self.researcher.websocket,
            )
        return content
    ...
    async def __get_similar_content_by_query_with_vectorstore(self, query, filter):
        if self.researcher.verbose:
            await stream_output(
                ...
                f"📚 מאחזר מידע רלוונטי בהסתמך על השאלה: {query}...",
                self.researcher.websocket,
            )
    ...
    async def __get_similar_written_contents_by_query(self,
        ...
        if self.researcher.verbose:
            await stream_output(
                ...
                f"🔎 מאזר מידע כתוב ורלוונטי לשאלה {query}...",
                self.researcher.websocket,
            )
