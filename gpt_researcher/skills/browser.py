    ...
    async def browse_urls(self, urls: List[str]) -> List[Dict]:
        ...
        if self.researcher.verbose:
            await stream_output(
                "logs",
                "scraping_urls",
                f"🌐 מאחזר טקסט רלוונטי מ {len(urls)} מקורות...",
        ...
        if self.researcher.verbose:
            await stream_output(
                "logs",
                "scraping_content",
                f"📄 מאחזר {len(scraped_content)} עמודים של טקסט",
                self.researcher.websocket,
            )
            await stream_output(
                "logs",
                "scraping_images",
                f"🖼️ נבחרו {len(new_images)} תמונות חדשות מ {len(images)} תמונות בסך הכל",
                self.researcher.websocket,
                True,
                new_images
            )
