...
const Accordion = ({ logs }: { logs: any[] }) => {
  ...
  return log.header === 'differences'
    ...
    : `📄 קורא טקסט מהלינק: ${sourceUrl}`;
};

const renderLogContent = (log: Log) => {
  if (log.header === 'differences' && log.processedData) {
    return log.processedData.map((data: ProcessedData, index: number) => (
      <div key={index} className="mb-4">
        <h3 className="font-semibold text-sm text-body-color dark:text-dark-6">{data.field}:</h3>
        {data.isMarkdown ? (
      ...
      {logs.map((log, index) => (
        <div key={index}>
          <h2 id={`accordion-collapse-heading-${index}`}>
            <button
              type="button"
              className="flex items-center w-full p-5 font-medium rtl:text-right text-sm text-white rounded-t-xl focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-800 hover:bg-gray-700 hover:text-white gap-3"
              onClick={() => handleToggle(index)}
              aria-expanded={openIndex === index}
              aria-controls={`accordion-collapse-body-${index}`}
            >
              <span className="flex text-right">{getLogHeaderText(log)}</span>
              <svg
                data-accordion-icon
