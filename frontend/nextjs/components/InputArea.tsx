...
const InputArea: FC<TInputAreaProps> = ({
  ...
  const placeholder = handleSecondary ? "שאלות המשך..." : "באיזה נושא תרצה שאבצע מחקר"
  return (
    <form
    ...
    >
      {
        ...
        <div
          ...
          className="relative flex h-[40px] w-[40px] sm:h-[50px] sm:w-[50px] shrink-0 items-center justify-center rounded-[3px] bg-[linear-gradient(154deg,#1B1B16_23.37%,#565646_91.91%)] disabled:pointer-events-none disabled:opacity-75"
          onClick={(e) =>{
            if (!disabled){
              e.preventDefault();
              if (reset) reset();
